...
public class RequerimientoDao_2 {
    ...
    public ArrayList<Requerimiento_2> requerimiento2 (Integer limit) throws SQLException{
        ...
        try{
            //Preparar la consulta y su conexión.
                    String consulta = "SELECT Fecha_Inicio as \"Fec Inicio\"," +
                              "Constructora as \"Nombre Constructora\"," +
                              " Ciudad as \"Nombre Ciudad\"," +
                              "T1.NombreApellido as \"Nombre Lider\"," +
                              "T2.Codigo_Tipo as \"Cod. Tipo\", T2.Estrato  FROM Proyecto as Pr " +
                              "LEFT JOIN (SELECT Li.ID_Lider, Li.Nombre, Li.Segundo_Apellido," +
                              "(Li.Nombre || ' ' || Li.Primer_Apellido) as NombreApellido " +
                              "FROM Lider as Li) T1 on Pr.ID_Lider = T1.ID_Lider " +
                                    "LEFT JOIN (SELECT Ti.ID_Tipo, Ti.Codigo_Tipo," +
                                    "Ti.Estrato FROM Tipo as Ti) T2 on Pr.ID_Tipo = T2.ID_Tipo " +
    ...
    public ArrayList<Requerimiento_2> requerimiento2 (String DateIni, String DateFin, String campoCiudad, Integer limit) throws SQLException{
        ...
        try{
            ...
            String consulta = "SELECT Fecha_Inicio as \"Fec Inicio\"," +
                              ...
                              "FROM Lider as Li) T1 on Pr.ID_Lider = T1.ID_Lider " +
                                    "LEFT JOIN (SELECT Ti.ID_Tipo, Ti.Codigo_Tipo," +
                                    "Ti.Estrato FROM Tipo as Ti) T2 on Pr.ID_Tipo = T2.ID_Tipo " +
                                    "WHERE Fecha_Inicio BETWEEN '" + DateIni + "' AND '" + DateFin + "'";
            if(campoCiudad != null){
                consulta = consulta +  " AND Ciudad = '" + campoCiudad + "'";
            }
